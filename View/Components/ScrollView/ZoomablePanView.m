...
//        self.layer.anchorPoint = CGPointMake(.5f, .5f);
    ...
    // to the x edge
    if (fabsf(oCenter.x - (point.x + view.transform.tx + oOffset.x)) >= fabsf(oCenter.x * view.transform.a - oCenter.x)) {
        //        return;
        point = CGPointMake(center.x , point.y);
    }
    // to the y edge
    if (fabsf(oCenter.y - (point.y + view.transform.ty)) >= fabsf(oCenter.y * view.transform.d - oCenter.y)) {
        //        return;
...
-(void)handlePan:(UIPanGestureRecognizer*)sender
{
    ...
    // to the x edge
    if (fabsf(oCenter.x - (point.x + view.transform.tx)) >= fabsf(oCenter.x * view.transform.a - oCenter.x)) {
        //        return;
        point = CGPointMake(center.x , point.y);
    }
    // to the y edge
